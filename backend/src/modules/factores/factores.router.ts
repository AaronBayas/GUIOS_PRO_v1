...
export async function factoresRouter(fastify: FastifyInstance) {
  ...
  fastify.get('/:id', {
    ...
    if (!factor) return reply.status(404).send({ error: 'Factor no encontrado' })
  ...
  fastify.put('/:id', {
    ...
    const body = req.body as {
      ...
      subfactores?: { subfactor_id?: number; subfactor_name: string; es_critico: boolean; activo?: boolean }[]
    }
    ...
    const currentFactor = await prisma.factor.findUnique({
      where: { factor_id: factorId },
      include: { subfactores: true },
    })
    if (!currentFactor) return reply.status(404).send({ error: 'Factor no encontrado' })
  ...
  fastify.post('/:id/subfactores', {
    ...
    const { id } = req.params as { id: string }
    const body = req.body as { subfactor_name: string; es_critico?: boolean }

    const factor = await prisma.factor.findUnique({ where: { factor_id: parseInt(id) } })
    if (!factor) return reply.status(404).send({ error: 'Factor no encontrado' })

    const maxOrden = await prisma.subfactor.aggregate({
      where: { factor_id: parseInt(id) },
      _max: { orden: true },
    })
  ...
  fastify.get('/dimensiones/list', {
    ...
    const dimensiones = await prisma.dimension.findMany({ orderBy: { orden: 'asc' } })
