...
async function iaRouter(fastify) {
    ...
    fastify.post('/analizar/:evaluacionId', {
        ...
        const existing = await database_1.prisma.aIRecomendacion.findUnique({ where: { evaluacion_id: evalId } });
        if (existing)
            return reply.send({ ai: existing });
        // Construir análisis (con o sin API key)
        let respuestaTexto;
        let modeloUsado;
        if (env_1.config.aiApiKey) {
            // Intentar con API de IA real
            try {
                const prompt = buildPrompt(evaluacion);
                const response = await callAI(prompt);
                respuestaTexto = response.text;
                modeloUsado = env_1.config.aiModel;
            }
            catch {
        ...
        const aiRec = await database_1.prisma.aIRecomendacion.create({
            data: {
...
async function callAI(prompt) {
    if (env_1.config.aiProvider === 'anthropic') {
        const response = await fetch('https://api.anthropic.com/v1/messages', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'x-api-key': env_1.config.aiApiKey,
                'anthropic-version': '2023-06-01',
            },
            body: JSON.stringify({
                model: env_1.config.aiModel,
                max_tokens: 2000,
        ...
        const response = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${env_1.config.aiApiKey}`,
            },
            body: JSON.stringify({
                model: env_1.config.aiModel,
                messages: [{ role: 'user', content: prompt }],
...
function generateLocalAnalysis(evaluacion) {
    ...
    const recTexto = (0, guiosad_engine_1.getDescripcionRecomendacion)(rec, evaluacion.software_nombre);
